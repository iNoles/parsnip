...
final class StandardTypeConverters {
    ...
    private static int rangeCheckInt(String strValue, String typeMessage, int min, int max)
        ...
        if (value < min || value > max) {
            throw new XmlDataException(String.format(ERROR_FORMAT, typeMessage, value));
        }
        return value;
    }
    ...
    static <T extends Enum<T>> TypeConverter<T> enumConverter(final Class<T> enumType) {
        return new TypeConverter<T>() {
            ...
            public T from(String value) {
                try {
                    ...
                    throw new XmlDataException("Expected one of " + Arrays.toString(enumType.getEnumConstants()) + " but was " + value);
                }
            }
